...
class TaskGen:
    ...
    def draw_hhess_s(self, n: int):
        return self.__mhhess + np.random.normal(
            0, self.__hess_sd, (n, self.__dim, self.__dim)
        )
    ...
    def draw_tasks(self, n) -> tuple[list[Task], np.ndarray, np.ndarray, np.ndarray]:
        ...
        return (
            [
                make_task(param, hhess, omega, self.__temp)
                for param, hhess, omega in zip(params, hhess_s, omegas)
            ],
            params,
            hhess_s,
            omegas,
...
def main(
    ...
    half_draw_cov = (
        np.diag(
            np.concatenate(
                [
                    np.exp(np.random.uniform(-0.5, 0.5, true_dim)),
                    np.full(d - true_dim, eps_false_dim),
                ]
            )
        )
        @ ortho_mat
    ...
    assert (
        np.sum(
            (np.linalg.eigvalsh(np.cov(task_gen.draw_params(10000), rowvar=False)))
            > (eps_false_dim**2) * 1.5
        )
        == true_dim
    )
    ...
    tic = time()
    mlearn.meta_learn(
        epochs=1, eta=1 / temp, kl_max=0.2, mini_batch_size=10
    )  # Initial training
    tac = time()
    print("-" * 10, f"Complete initial training in {tac-tic} s", "-" * 10)

    # Change hyperparameters
    mlearn.hyperparams.update(
        {"per_step": [20, 0] * 2, "chain_length": 4, "dampen": 0.3}
    )
    tic = time()
